...
public class LetterBoard : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler
{
    ...
    private void FoundWord(string word, List<LetterTile> letterTilesForWord)
    {
        ...
        CheckIfYouCanCompleteThePuzzle(word);
    }
    ...
    private string _attemptedWord;

    void CheckIfYouCanCompleteThePuzzle(string foundWord)
    {
        if (_foundWordsIndexes == null)
            _foundWordsIndexes = new List<int>();

        for (var i = 0; i < currentWords.Count; i++)
        {
            if (_foundWordsIndexes.Contains(i))
                continue;
            if (currentWords[i] == foundWord)
            {
                _foundWordsIndexes.Add(i);
                continue;
            }
            CheckWordCompletion(currentWords[i]);
        }
    }

    void CheckWordCompletion(string word)
    {
        _wordToCheck = word;
        var startsWith = word[0].ToString().ToUpper();

        var sLetterTiles = letterTiles.FindAll(tile => tile != null && tile.Found == false && tile.LetterText.text == startsWith);

        bool isAtleastOneCombinationPossible = false;
        foreach (LetterTile letterTile in sLetterTiles)
        {
            ...
            if (isAtleastOneCombinationPossible)
            {
                Debug.LogError(" Word can be completed ! | " + _wordToCheck + " : " + _attemptedWord);
                break;
            }
        }

        if (isAtleastOneCombinationPossible == false)
            SetImpossible();
    }

    void SetImpossible()
    {
        Debug.LogError("Puzzle is imposible !!");
        StopPanel.SetActive(true);
        RefreshButtonHighlight.SetActive(true);
        _foundWordsIndexes = new List<int>();
    ...
    bool ConnectTile(LetterTile tile, int wordIndex)
    {
        ...
        if (_wordToCheck.Length == (wordIndex + 1)
            && _wordToCheck == _attemptedWord)
        {
            return true;
        }
